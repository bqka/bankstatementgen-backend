...
public class SbiTemplate2 implements BankPdfTemplate {
    ...
    public byte[] generate(Statement stmt) throws Exception {
        ...
        writer.setEncryption(
            stmt.meta.password == null
                ? stmt.details.accountNumber.getBytes()
                : stmt.meta.password.getBytes(),
            null,
            0,
        ...
        info.put(
            PdfName.PRODUCER,
            new PdfString(
                "iText 2.0.4 (by lowagie.com); modified using iText® 5.5.10 ©2000-2015 iText Group NV (AGPL-version)"
            )
        );
        setPdfDates(writer);
        ...
        float startY = 806f;
        float marginTop =
            pageHeight - startY + 54f + (15 + addrlines) * 13.5f + 3 * 18f;
    ...
    private void addTransactions(Document doc, Fonts fonts, Statement stmt)
        ...
        for (Transaction tx : stmt.transactions) {
            table.addCell(
                bodyCell(format(tx.date), fonts.body, Element.ALIGN_RIGHT)
            );
            table.addCell(
                bodyCell(format(tx.date), fonts.body, Element.ALIGN_RIGHT)
            );
            table.addCell(
                bodyCell(tx.description, fonts.body, Element.ALIGN_LEFT)
            );
            table.addCell(bodyCell("", fonts.body, Element.ALIGN_LEFT));
            table.addCell(
                bodyCell(amount(tx.debit), fonts.body, Element.ALIGN_RIGHT)
            );
            table.addCell(
                bodyCell(amount(tx.credit), fonts.body, Element.ALIGN_RIGHT)
            );
            table.addCell(
                bodyCell(amount(tx.balance), fonts.body, Element.ALIGN_RIGHT)
            );
        }
    ...
    private static final DateTimeFormatter SBI_FORMAT =
